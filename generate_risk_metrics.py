...
def main():
    ...
    drawdowns = []
    dd_details = []
    for _i, trial in enumerate(all_trials):
    ...
    cvar_95 = (
        float(np.mean(sorted_evs[sorted_evs <= var_95])) if np.any(sorted_evs <= var_95) else var_95
    )
    cvar_99 = (
        float(np.mean(sorted_evs[sorted_evs <= var_99])) if np.any(sorted_evs <= var_99) else var_99
    )
    ...
    pct_data = {
        p: [np.percentile(ev_matrix[:, yr], p) for yr in range(n_years)] for p in percentiles
    }
    ...
    for lo, hi, color, label in bands:
        ax.fill_between(
            years,
            np.array(pct_data[lo]) / CHO,
            np.array(pct_data[hi]) / CHO,
            color=color,
            alpha=0.8,
            label=label,
        )
    ...
    ax.fill_between(
        years,
        np.array(var_99_by_year) / CHO,
        np.array(var_95_by_year) / CHO,
        color="#FFCDD2",
        alpha=0.6,
        label="VaR 95-99% band",
    )
    ...
    ax.axvline(
        np.percentile(drawdowns, 90) * 100,
        color="#B71C1C",
        linewidth=2,
        linestyle=":",
    ...
    ax.plot(
        years, np.median(ev_matrix, axis=0) / CHO, color="#1565C0", linewidth=2.5, label="Median"
    )
